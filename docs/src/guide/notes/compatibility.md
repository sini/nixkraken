...
This project is still in development. It is not _early stage_ per se, but still is not stable enough to be considered "production ready".
...
**Users seeking stability should use aforementioned release branches** rather than the `main` branch. There is also the `stable` branch, which dynamically tracks the latest working release. No development will occur on the `stable` branch and it will never introduce a feature or bug fix which is not available on the latest available release branch.

To use a specific release branch, modify `flake.nix`'s inputs like this:
...
Users are also encouraged to **[report any issues][gh-issues]** encountered, or even better: to **contribute fixes**! [Pull requests][gh-prs] are always welcome 🙂
