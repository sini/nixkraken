...
Currently, only the [OAuth][oauth] authentication flow is supported, with the following providers:
...
When NixKraken is [enabled][doc-opt-enable], the `gk-login` package is automatically added to [`home.packages`][hm-opt-home-pkgs], making it available globally for your user.
