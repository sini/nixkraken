...
for i, machine in enumerate([machine1, machine2], start=1):
    machine.wait_for_x()

    with subtest(f"GitKraken theme is customized (machine{i})"):
        ...
        t.assertTrue(isDarkScheme(os.path.join(machine.out_dir, f"snapshot-{i}.png")))
