...
let
  defaultVariants = {
    dark = "dark-color-blind.jsonc";
    light = "light-color-blind.jsonc";
  };
...
stdenvNoCC.mkDerivation rec {
  name = "gitkraken-theme-color-blind";
  version = "1.4.0";
  ...
  installPhase = lib.concatLines (
    lib.flatten [
      ...
      ''
      (lib.map (
        variant: "cp 'Themes/Default Themes Modified/${defaultVariants.${variant}}' $out"
      ) variants)
      "runHook postInstall"
    ]
  );
  ...
  meta = {
    description = "Color blind theme for GitKraken";
    homepage = "https://github.com/jonbunator/gitkraken-custom-themes/tree/v${version}/Themes/Default%20Themes%20Modified";
